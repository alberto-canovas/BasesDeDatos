...
4.0.1. 📝Ejercicios sobre Funciones❓:

Crea una función llamada cuentaLetra() que dado una cadena de caracteres recibida como primer parámetro, llamada pCadena, y un segundo parámetro 
llamado pLetra que contendrá un único carácter. Devuelva como resultado el número de apariciones de dicho carácter en la cadena, utilizando 
un bucle para su elaboración.


DELIMITER $$
DROP FUNCTION IF EXISTS cuentaLetra$$
CREATE FUNCTION cuentaLetra(
    pCadena varchar(255),
    pLetra varchar(1)
) RETURNS INTEGER     
-- se pone con returnS porque es para indicar el tipo de dato que tiene que devolver la función(string,integer...)

BEGIN
    DECLARE vI INTEGER DEFAULT 1; -- contador para que empiece en la posición 1 de la cadena
    DECLARE vLongitud INTEGER;
    DECLARE vCuenta INTEGER DEFAULT 0; -- ponemos el 0 para que empiece a contar desde 0, sino sería null

    
    -- SELECT length(pCadena) INTO vLongitud; ES LO MISMO
    SET vLongitud = length(pCadena);


    WHILE (vI <= vLongitud) DO
        IF SUBSTRING(pCadena,vI,1) = pLetra THEN 
        -- substring (1º(cadena),2º(iterador que va aumentando),3º(número de caracteres que avanza))
            SET vCuenta = vCuenta +1;
        END IF; 
    ...
    RETURN vCuenta;
/*
SELECT cuentaLetra('Murcielago marciano', 'm');
SELECT cuentaLetra('Murcielago marciano', 'x');
SELECT cuentaLetra(null, 'm');
*/
END $$

-- BUSCA POSICIÓN DE LETRA. FUNCIÓN NUEVA QUE DEVUELVE LA POSICIÓN DE LA LETRA.


DELIMITER $$
DROP FUNCTION IF EXISTS buscaPosLetra$$
CREATE FUNCTION buscaPosLetra(
    pCadena varchar(255),
    pLetra varchar(1)
) RETURNS varchar(1000) COMMENT 'Devuelve una cadena formada por las posiciones donde se encuentra la letra en la cadena separadas por coma'     
-- se pone con returnS porque es para indicar el tipo de dato que tiene que devolver la función(string,integer...)

BEGIN
    DECLARE vI INTEGER DEFAULT 1; -- contador para que empiece en la posición 1 de la cadena
    DECLARE vLongitud INTEGER;
    DECLARE vResultado varchar(1000) DEFAULT ''; -- el mismo tipo del valor que voy a devolver en este caso
    ...
    WHILE (vI <= vLongitud) DO
        ...
        -- substring (1º(cadena),2º(iterador que va aumentando),3º(número de caracteres que avanza))
            IF vResultado = '' THEN
                SET vResultado = vI;
                -- SET vResultado = convert(vI,varchar(16)); -- como el vI es Integer y el vResultado es varchar hay que convertir el vI en varchar
        ELSE
            SET vResultado = concat(vResultado,',',vI);
            END IF; 
        END IF;
    ...
    RETURN vCuenta;
/*
SELECT buscaPosLetra('Murcielago marciano', 'm');
SELECT buscaPosLetra('Murcielago marciano', 'x');
SELECT buscaPosLetra(null, 'm');
*/
END $$



-- transformación de la función anterior en un procedimiento 

DELIMITER $$
DROP PROCEDURE IF EXISTS ProcbuscaPosLetra$$
CREATE PROCEDURE ProcbuscaPosLetra(
    pCadena varchar(255),
    pLetra varchar(1),
    OUT pResultado varchar(1000))
...
BEGIN
    ...
    SET pResultado = vResultado;
/*
call ProcbuscaPosLetra('Murcielago marciano', 'm', @v1);
call ProcbuscaPosLetra('Murcielago marciano', 'x', @v2);
call ProcbuscaPosLetra(null, 'm', @v3);
...
Crea un procedimiento de nombre producto_getDescripcion al que se le pase como parámetro el código de un producto id y 
devuelve en el mismo parámetro el nombre del fabricante, seguido de : y el nombre completo de dicho producto.
En este procedimiento se pide que el mismo parámetro sirva para enviar el código del producto (que es integer en la base de datos) y 
que devuelva el nombre del fabricante ('DESCONOCIDO' si es nulo), seguido de : y el nombre completo del producto (que será varchardel tamaño de la 
columna Producto.nombre sumado el tamaño de la columna nombre en la tabla producto con el tamaño la suma de fabricante.nombre más tres unidades.
...
DROP PROCEDURE IF EXISTS producto_getDescripcion $$
CREATE PROCEDURE producto_getDescripcion(
    INOUT pdato varchar(153)
)
BEGIN
    DECLARE vId int;
    DECLARE vdato varchar(153);
	DECLARE vResultado varchar(153);
    
    SET vId = convert(pdato,integer);

    SELECT concat(ifnull(f.nombre,'Desconocido'), ' : ',p.nombre)
        INTO vResultado
    	FROM producto p LEFT JOIN fabricante f
        ON p.código_fabricante = f.código
    	WHERE p.id = vId;
